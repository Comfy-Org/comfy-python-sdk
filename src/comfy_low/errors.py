...
class AssetInUse(ApiError):
    ...
    The ``409`` on ``DELETE /api/v2/assets/{id}``. Deliberately opaque about
    *which* hold applies — the spec says the body never names it — so the class
    is a signal to stop retrying the delete, not to inspect ``details``.
    Distinct from :class:`HashMismatch`, the other documented ``409``, which is
    an upload rejecting bytes that did not match ``expected_hash``.
    """
